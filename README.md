# TV autofocus issues in SDK 54 and new architecture

There are 2 main issues:

- on tvOS when enabling autoFocus on a TVFocusGuideView, the focus gets lost somewhere. Reloading solves the issue.
- on Android, when trying to keep focus on a transparentModal the focus keeps moving to the first screen in the stack. I can force it to focus back with a requestTVFocus, but you still get all the events and styling which are attached to the focus moving.

You can recreate the tvOS bug by setting autoFocus to true.

The android bug is also visible in this example. There is an onFocus which gets triggered when you navigate back to the stack. If I don’t add the useFocusEffect that forces the focus on the focusview, the focus will stay on the main screen.
